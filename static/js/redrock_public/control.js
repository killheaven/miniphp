	...
	$.ajax({
		   ...
		   success: function(msg){
				if(msg.status){
					...
					var d=art.dialog({
						lock: true,
						content:"<div style='max-width:800px;'>"+contents+"</div>",
						width:800,
						height:100,
						padding:20,
						fixed: false,
...
if(allObj){
	...
	//$(this).parents('.'+articleBox).append(reply).append(reply).append(reply);
	$(this).unbind('click');
		...
		$('.replyComment-button').bind('click',function(){
			...
			*/
			...
			if(status){
				...
				//var reply ='<div class="messages-replyBox"><div class="messages-replyBox-avatar"><a href=""><img src="dfa/avatar.php?uid=123&size=big"/></a></div><p><span class="messages-replyBox-name">哈哈:</span>sdfasdfasdfasf<span><a href="#reply">回复<span></span></a></span></p><div class="messages-reply"><div class="messages-date">201123</div> </div></div>'
				//$(this).parent().append(reply);
				insertReply(id,uid,replyText,$(this).parent());
		   ...
		   success: function(msg){
				if(msg.status){
					...
					if(content.content){
					  ...
					  var add=0;
					  _this.children('.content-li-like').nextAll('div').each(function(){
						//alert(parseInt($(this).outerHeight()));
						add+=parseInt($(this).outerHeight())+5;
						//alert(add);
					    });
						_this.nextAll('.waterfall-box').each(function(){
							if($(this).css('left')== left){
								$(this).css('top',function(index,value){ return parseInt(value)+add+'px';});
							}
						})
...
function insertReply(id,uid,content,_this){
	$.ajax({
		   type: "POST",
		   url: "index.php?c=commentajax&m=replyMsg",
		   data: "action=replyMsg&id="+id+"&content="+content+"&userId="+uid+"",
		   dataType:'json',
		   success: function(msg){
		  // alert(msg);
				if(msg.status){
					var reply ='<div class="messages-replyBox"><div class="messages-replyBox-avatar"><a href=""><img src="'+msg.uc_api+'/avatar.php?uid='+msg.uid+'&size=big"/></a></div><p><span class="messages-replyBox-name">'+msg.user+':</span>'+msg.content+'<span><a href="#reply">回复<span></span></a></span></p><div class="messages-reply"><div class="messages-date">'+msg.time+'</div> </div></div>';
				_this.append(reply);
				var left = _this.css('left');
				var replyHeight = parseInt(_this.children().last().outerHeight())+10;
				_this.nextAll('.waterfall-box').each(function(){
					if($(this).css('left')== left){
						alert(replyHeight);
						$(this).css('top',function(index,value){ return parseInt(value)+replyHeight+'px';});
					}
				})
				}else{
					if(msg.errorId==0){

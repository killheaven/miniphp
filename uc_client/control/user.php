...
define('UC_USER_USERNUMBER_FORMAT_ILLEGAL', -7);
define('UC_USER_USERNUMBER_EXISTS', -8);
define('UC_USER_PROFILE_MISMATCH', -9);
...
class usercontrol extends base {
	...
	function onregister() {
		...
		$usernumber = $this->input('usernumber');
		$realname = $this->input('realname');
		$usertype = $this->input('usertype');
		$idnum = $this->input('idnum');
		$questionid = $this->input('questionid');
		$answer = $this->input('answer');
		$regip = $this->input('regip');

		$usernumber = strtolower($usernumber);
		$idnum = strtolower($idnum);

		if(($status = $this->_check_username($username)) < 0 ||
		   ($status = $this->_check_userinfo(&$email, &$usernumber, &$realname ,&$usertype, $idnum, 1)) < 0) {
			return $status;
		}

		$uid = $_ENV['user']->add_user($username, $password, $email, $usernumber, $realname, $usertype, 0, $questionid, $answer, $regip);
		return $uid;
	}
	...
	function onedit() {
		...
		if(($usernumber || $realname || $usertype) && ($status = $this->_check_userinfo(&$email, &$usernumber, &$realname, &$usertype)) <0){
			return $status;
		}
		$status = $_ENV['user']->edit_user($username, $oldpw, $newpw, $email, $usernumber, $realname, $usertype, $ignoreoldpw, $questionid, $answer);
	...
	function onlogin() {
		...
		if($isuid == 1) {
			...
			$user = $_ENV['user']->get_user_by_usernumber($username);
		} else {
		...
		if(empty($user)) {
			...
			$status = $user['uid'];
			$_ENV['user']->login_update($user['uid']);
		}
		$merge = $status != -1 && !$isuid && $_ENV['user']->check_mergeuser($username) ? 1 : 0;
		return array($status, $user['username'], $password, $user['email'], $merge, $user['usernumber'], $user['realname'], $user['usertype']);
	}
	...
	function oncheck_userinfo(){
		...
		$usertype = $this->input('usertype');
		$idnum = $this->input('idnum');
		return $_ENV['user']->check_userinfo($email, $usernumber, $realname, $usertype, $idnum) ? 1 : 0;
	}
	...
	function onget_user() {
		...
		if($isuid == 1) {
			...
			$status = $_ENV['user']->get_user_by_usernumber($username);
		} elseif($isuid == 3) {
		...
		if($status) {
			return array($status['uid'],$status['username'],$status['email'],$status['usernumber'],$status['realname'],$status['usertype'],$status['password']);
		} else {
			return 0;
		}
	}
	...
	function _check_userinfo(&$email, &$usernumber, &$realname ,&$usertype, $idnum ='', $needidnum = 0) {
		if(empty($this->settings)) {
			$this->settings = $this->cache('settings');
		}
		if(!$this->settings['trueinfo']) {
			if(!$_ENV['user']->check_usernumberformat($usernumber)) {
				return UC_USER_USERNUMBER_FORMAT_ILLEGAL;
			} elseif($_ENV['user']->check_usernumberexists($usernumber)) {
				return UC_USER_USERNUMBER_EXISTS;
			} else {
				return $this->_check_email($email);
			}
		} elseif($needidnum && !empty($idnum) && !$_ENV['user']->check_idnumformat($idnum)) {
			return UC_USER_PROFILE_MISMATCH;
		} elseif(!$_ENV['user']->check_userinfo(&$email, &$usernumber, &$realname ,&$usertype ,$needidnum ? $idnum : "")) {
			return UC_USER_PROFILE_MISMATCH;
		} elseif($_ENV['user']->check_usernumberexists($usernumber)) {
			return UC_USER_USERNUMBER_EXISTS;
		} else {

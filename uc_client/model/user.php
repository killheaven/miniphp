...
class usermodel {
	...
	function get_user_by_usernumber($usernumber) {
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE usernumber='$usernumber'");
		return $arr;
	}
	...
	function check_usernumberformat($usernumber) {
		return preg_match("/^(s\d{9}|\d{6}(\d{2}|\d{4})?)?$/", $usernumber);
	}

	function check_usernumberaccess($usernumber, $realname) {
		if ($usernumber == '') {
			return FALSE;
		}
		$sqladd = $realname !== '' ? "AND realname='$realname'" : '';
		$usernumber = $this->db->result_first("SELECT usernumber FROM ".UC_DBTABLEPRE."userinfo WHERE usernumber='$usernumber' $sqladd");
		return $usernumber;
	}

	function check_usernumberexists($usernumber) {
		if ($usernumber == '') {
			return FALSE;
		}
		$usernumber = $this->db->result_first("SELECT usernumber FROM  ".UC_DBTABLEPRE."members WHERE usernumber='$usernumber' ");
		return $usernumber;
	}

	function check_realnameaccess($realname) {
		$username = $this->db->reault_first("SELECT usernumber FROM ".UC_DBTABLEPRE."userinfo WHERE realname='$realname'");
		return $username;
	}
	...
	function check_idnumformat($idnum){
		return preg_match("/\d{5,}[x\d]$/", $idnum);
	}

	function check_userinfo(&$email, &$usernumber, &$realname ,&$usertype ,$idnum ='') {
		$sqladd = $email !== '' ? "AND email='$email'" : '';
		$sqladd .= $usernumber !== '' ? "AND usernumber='$usernumber'" : '';
		$sqladd .= $realname !== '' ? "AND realname='$realname'" : '';
		$sqladd .= $usertype !== '' ? "AND usertype='$usertype'" : '';
		$sqladd .= $idnum !== '' ? "AND idnum like '%$idnum'" : '';

		if ($sqladd == ''){
			return TRUE;
		}
		
		$userinfo = $this->db->fetch_first("SELECT email,usernumber,realname,usertype FROM ".UC_DBTABLEPRE."userinfo WHERE 1 $sqladd");
		
		$email = $userinfo['email'] !== '' ? $userinfo['email'] : $email;
		$usernumber = $userinfo['usernumber'] !== '' ? $userinfo['usernumber'] : $usernumber;
		$realname = $userinfo['realname'] !== '' ? $userinfo['realname'] : $realname;
		$usertype = $userinfo['usertype'] !== '' ? $userinfo['usertype'] : $usertype;

		return $userinfo;
	}

	function login_update($uid) {
		$sqladd = 'lastloginip="'.$this->base->onlineip.'", ';
		$sqladd .= 'lastlogintime="'.$this->base->time.'"';
		$this->db->query("UPDATE ".UC_DBTABLEPRE."members SET $sqladd WHERE uid='$uid'");
		return $this->db->affected_rows();
	}

	function add_user($username, $password, $email, $usernumber, $realname, $usertype, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		$regip = empty($regip) ? $this->base->onlineip : $regip;
		$salt = substr(uniqid(rand()), -6);
		$password = md5(md5($password).$salt);
		$sqladd = $uid ? "uid='".intval($uid)."'," : '';
		$sqladd .= $questionid > 0 ? " secques='".$this->quescrypt($questionid, $answer)."'," : " secques='',";
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd username='$username', password='$password', email='$email', usernumber='$usernumber', realname='$realname', usertype='$usertype', regip='$regip', regdate='".$this->base->time."', salt='$salt'");
		$uid = $this->db->insert_id();
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."memberfields SET uid='$uid'");
		return $uid;
	}

	function edit_user($username, $oldpw, $newpw, $email, $usernumber, $realname, $usertype, $ignoreoldpw = 0, $questionid = '', $answer = '') {
		...
		$sqladd .= $usernumber ? ($sqladd ? ',' : '')." usernumber='$usernumber'" : '';
